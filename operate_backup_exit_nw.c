...
int paths_compare(t_path *path, t_path *new_path)//, t_info *info)
{
	t_path *temp;
	t_path *new_temp;
	...
	while (temp || new_temp)
	{
		if ((temp && !new_temp) || (!temp && new_temp))
			return (0);//not equal
		if (ft_strcmp(temp->actual->name, new_temp->actual->name))
		{
			return (0);//not equal
		}
		// temp = temp->next;
...
void	operate(t_info *info)
{
	...
	//for (int i = 0; i < 17; i++)
	{
		...
		else
		{
			//t_path *ew;
			// //change then
			//ew = shortest_path(info);
			
			// //==============
			// if (paths_compare(info->paths->actual_path, ew))
			// //info->paths->next = new_path_list(shortest_path(info));
			// {
			// 	//printf("original path\n");
			// 	info->paths->next = new_path_list(ew);
			...
			t_path *ew;
			ew = shortest_path(info);
			if (paths_compare(info->paths->actual_path, ew))//, info))
			//info->paths->next = new_path_list(shortest_path(info));
			{
				//printf("original path\n");
				info->paths->next = new_path_list(ew);
